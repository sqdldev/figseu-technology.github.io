...
let data;
let reqId;
$.getJSON('Delta60_6Yaj6fWSr5wJJznWEjHy224.rse', d => {
  data = d;
  loadLevel(level);
  $('#play').show();
  $('#PresentCode').hide();
  ...
  $('#stars').html('　');
  $('#Waves').html('Λ × ' + $.cookie('delta_60_wave'));
...
function start(e) {
  ...
  if (!started) {
    ...
    if (typeof wave == 'undefined') {
        wave = 0;
        $.cookie('delta_60_wave', wave, { expires: 40, domain:'figseu-technology.github.io'});
    }
    $('#zero').get(0).play();
    wave++;
    reset();
...
function gameover() {
  ...
  $.cookie('delta_60_wave', wave, { expires: 400, domain:'figseu-technology.github.io'});
  started = false;
  ball.speed.z = 0;
  $('#main').fadeIn(100);
  $('#retry').hide();
