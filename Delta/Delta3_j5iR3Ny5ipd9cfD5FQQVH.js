...
let data;
let reqId;
$.getJSON('Delta3_j5iR3Ny5ipd9cfD5FQQVH.rse', d => {
  data = d;
  loadLevel(level);
  $('#play').show();
  $('#PresentCode').hide();
  ...
  $('#level').html('The Almandine');
  $('#stars').html('　');
  $('#Waves').html('Δ × ' + $.cookie('delta_two_wave'));
...
function start(e) {
  ...
  if (!started) {
    ...
    if (typeof wave == 'undefined') {
        wave = 0;
        $.cookie('delta_two_wave', wave, { expires: 40, domain:'figseu-technology.github.io'});
    }
    $('#zero').get(0).play();
    wave++;
    reset();
...
function gameover() {
  ...
  $.cookie('delta_two_wave', wave, { expires: 400, domain:'figseu-technology.github.io'});
  started = false;
  ball.speed.z = 0;
  $('#main').fadeIn(100);
  $('#retry').hide();
