...
let data;
let reqId;
$.getJSON('Delta13_XG9WmaheuETEMrwnBgCLEbgQDBTZBVLt.rse', d => {
  data = d;
  loadLevel(level);
  $('#play').show();
  $('#PresentCode').hide();
  ...
  $('#level').html('Alone Remix');
  $('#stars').html('　');
  $('#Waves').html('Δ × ' + $.cookie('delta_13_wave'));
...
function start(e) {
  ...
  if (!started) {
    ...
    if (typeof wave == 'undefined') {
        wave = 0;
        $.cookie('delta_13_wave', wave, { expires: 40, domain:'figseu-technology.github.io'});
    }
    $('#zero').get(0).play();
    wave++;
    reset();
    ...
    for (var j in data[index].data[i]) {
      switch (data[index].data[i][j]) {
        ...
        case 4:
          ...
          world.push(new RubyCrystal(j - 2, -i, data[index].obstacle));
          break;
        case 5:
          world.push(new Mat(j - 2, -i, data[index].mat));
        ...
        case 9:
          ...
          break;
        case 10:
          world.push(new AloneMat(j - 2, -i, data[index].mat));
          break;
        case 11:
...
function gameover() {
  ...
  $.cookie('delta_13_wave', wave, { expires: 400, domain:'figseu-technology.github.io'});
  started = false;
  ball.speed.z = 0;
  $('#main').fadeIn(100);
  $('#retry').hide();
