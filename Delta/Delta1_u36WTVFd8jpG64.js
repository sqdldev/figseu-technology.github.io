...
let data;
let reqId;
$.getJSON('Delta1_u36WTVFd8jpG64.rse', d => {
  data = d;
  loadLevel(level);
  $('#play').show();
  $('#PresentCode').hide();
  ...
  $('#level').html('Time Crystal');
  $('#stars').html('　');
  $('#Waves').html('Δ × ' + $.cookie('delta_one_wave'));
...
function start(e) {
  ...
  if (!started) {
    ...
    if (typeof wave == 'undefined') {
        wave = 0;
        $.cookie('delta_one_wave', wave, { expires: 40, domain:'figseu-technology.github.io'});
    }
    $('#zero').get(0).play();
    wave++;
    reset();
...
function loadLevel(level) {
  const index = level - 1;
  renderer.THREE.ImageUtils.loadTexture( 'textures/163_20221230123545.cmr' );
...
function gameover() {
  ...
  $.cookie('delta_one_wave', wave, { expires: 400, domain:'figseu-technology.github.io'});
  started = false;
  ball.speed.z = 0;
  $('#main').fadeIn(100);
  $('#retry').hide();
