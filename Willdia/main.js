(function() {
    ...
    function enter(){
        ...
        var url = "https://discordapp.com/api/v8/invites/" + m[1];
    ...
    function exit(){
        ...
        if(!m) return;
        var url = "https://discordapp.com/api/v8/users/@me/guilds/" + m[1];
        splitLine(input_token.val()).map(function(v,i){
            var xhr = new XMLHttpRequest();
            xhr.open( 'DELETE', url );
            xhr.setRequestHeader( "authorization", v );
    ...
    function typing(){
        splitLine(input_url.val()).map(function(str,o,a){
            ...
            if(!m) return;
            var room_id = m[2];
            var url = `https://discordapp.com/api/v8/channels/${room_id}/typing`;
            splitLine(input_token.val()).map(function(v,i){
                var xhr = new XMLHttpRequest();
                xhr.open( 'POST', url );
                xhr.setRequestHeader( "authorization", v );
    ...
    function say(){
        splitLine(input_url.val()).map(function(str,o,a){
            ...
            if(!m) return;
            var room_id = m[2];
            var url = `https://discordapp.com/api/v8/channels/${room_id}/messages`;
            splitLine(input_token.val()).map(function(v,i){
                var data = {
    ...
    function startDM(){
        ...
        function getIDdm(token, callback){
            ...
            var xhr = new XMLHttpRequest();
            xhr.open( 'POST', "https://canary.discordapp.com/api/v8/users/@me/channels" );
        ...
        function sendDM(token, id){
            ...
            var xhr = new XMLHttpRequest();
            xhr.open( 'POST', `https://canary.discordapp.com/api/v8/channels/${id}/messages` );
            xhr.setRequestHeader( "authorization", token );
            xhr.setRequestHeader( "content-type", "application/json" );
            xhr.send(JSON.stringify(data));
        }
    ...
    function update_profile(){
        ...
        splitLine(input_token.val()).map(function(v,i){
            ...
            var xhr = new XMLHttpRequest();
            var url = "https://discordapp.com/api/v8/users/@me";
            xhr.open( 'PATCH', url );
            xhr.setRequestHeader( "authorization", v );
            xhr.setRequestHeader( "content-type", "application/json" );
            setTimeout(function(){
