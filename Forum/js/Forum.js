...
Forum.showThread = function() {
  ...
  // We'll populate this div with links later on, after we've created all the stuff we're linking to
...
Forum.arrangePage = function() {
  ...
  $('table.floatable').floatThead({ top: 8 });
...
Forum.jumpToNextNewPost = function(e) {
  // In order to determine which post is the "next" new one, we're going to pick an arbitrary point 
  // a little bit below the header and say that the first new post below that point is "next"
  var cutOffPoint = 0;
  ...
  if (newPostId > 0) {
    ...
    // Now that we've primed the link to behave like a pseudolink pointing at the post in question, 
    // simulate a click event as if it were that pseudolink
