...
class Bouncer {
  ...
  constructor(xpos, zpos, color) {
    color = parseInt(color);
    this.geometry = new THREE.BoxGeometry(1, 0.2, 1);
    this.material = new THREE.MeshBasicMaterial({ color: 0x007CFF });
    this.mesh = new THREE.Mesh(this.geometry, this.material);
    this.edgesGeometry = new THREE.EdgesGeometry(this.geometry);
...
class Points {
  constructor(xpos, zpos, color) {
    color = parseInt(color);
    this.geometry = new THREE.BoxGeometry(0.3, 1.5, 0.3);
    this.material = new THREE.MeshPhongMaterial({ color: 0xFF0018 });
    this.mesh = new THREE.Mesh(this.geometry, this.material);
    this.edgesGeometry = new THREE.EdgesGeometry(this.geometry);
    this.edgesMaterial = new THREE.LineBasicMaterial({ color: 0xFF0018 });
  ...
  detect() {
    if (
      ...
      ball.mesh.position.x <= this.mesh.position.x + 10 &&
      ball.mesh.position.z >= this.mesh.position.z - 1.8 &&
      ball.mesh.position.z <= this.mesh.position.z + 1.8 &&
      ball.mesh.position.z <= 1.8 &&
      ball.mesh.position.y < this.mesh.position.y + 8
    ) return true;
  }
}
